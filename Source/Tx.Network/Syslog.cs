...
namespace Tx.Network
{
    ...
    public class Syslog : UdpDatagram
    {
        ...
        public Syslog(UdpDatagram ReceivedPacket, Regex Parser) : base(ReceivedPacket)
        {
            ...
            SetRegex(Parser);
            string Message = Encoding.ASCII.GetString(UdpData); //should add support for other encoding per IETF RFC.
            Match matchMe = Parser.Match(Message);
            ...
            var pval = Regex.Match(Message, @"\<(?<PRIVAL>\d+?)\>\s*");
