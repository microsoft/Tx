...
using Tx.Binary;
    ...
    {
        public readonly Dictionary<string, string> TypeNames;
        ...
        public Dictionary<Type, EventStatistics> GetTypeStatistics(TypeCache typeCache, string inputFile)
        {
            ...
            foreach (var c in counts)
            {
                var manifest = typeCache.Manifests
                    .Where(m => m.ManifestId == c.ManifestId)
                    .FirstOrDefault();

                var line = manifest.Manifest.Split('\n')
                    .Where(l => l.Trim().StartsWith("struct")).FirstOrDefault();

                var className = line.Split(new char[] {' '}, StringSplitOptions.RemoveEmptyEntries)[1].Trim();

                Type type = typeCache.Types.Where(t => t.Name == className).FirstOrDefault();
        ...
        public void WriteToSummaryFile(string file, string fileName, string fileDirectory, Dictionary<string, Stats> statsDictionary)
        {
